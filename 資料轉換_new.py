        ...
        try:
            if isinstance(final_product_lst[i]['報酬率'][0], float):
                if (final_product_lst[i]['報酬率'][0] * cost >= exp_returns) and (cost > final_product_lst[i]['商品國外價格'][0]):
                    ...
                    return_pro['所需購買數量'] = round(final_product_lst[i]['報酬率'][0],3)
                    return_lst.append(return_pro)
        except KeyError:
            pass
        try:
            if isinstance(final_product_lst[i]['報酬率'][0], list):
                for j in final_product_lst[i]['報酬率']:
                    if (j[1] * cost >= exp_returns) and (cost > j[1]):
                        return_pro = {}
                        return_pro['商品名'] = final_product_lst[i]['商品中文名稱'] + ', 容量：' + j[0]
                        return_pro['報酬率'] = round(j[1],3)
                        return_lst.append(return_pro)
        except KeyError:
            pass
    return return_lst
